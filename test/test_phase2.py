...
if __name__ == "__main__":
    ...
    def update_and_compile(ip):
        ...
        cmd_cd = 'cd secure-distributed-linear-regression; pwd; git stash; git checkout master; git pull; make OBLIVCC=~/obliv-c/bin/oblivcc ; killall -9 test_linear_system'
    ...
    for alg in ['cgd', 'cholesky']:
        for sigma in [0.1]:
            for d in [10, 20, 50, 100]:
                for n in [d, 2*d, 5*d, 10*d, 100*d]:
                    print n, d, sigma, alg, num_iters_cgd
                    for i in range(args.num_examples):
                        filename_ls_in = 'test_LS_{0}x{1}_{2}_{3}.in'.format(n, d, sigma, i+5)
                        ...
                        for party in [1, 2]:
                            filename_ls_exec = 'test_LS_{0}x{1}_{2}_{3}_{4}_{5}_p{6}.exec'.format(n, d, sigma, i+5, alg, num_iters_cgd, party)
                            filepath_ls_exec = os.path.join(
                                args.dest_folder, filename_ls_exec)
                            filename_ls_out = os.path.splitext(filename_ls_exec)[0] + '.out'
                            filepath_ls_out = os.path.splitext(filepath_ls_exec)[0] + '.out'
                            ...
                            run_remotely(args.dest_folder, filepath_ls_in,
                                filename_ls_in, public_ips[1], cmd)
