...
if __name__ == "__main__":
    ...
    def update_and_compile(ip):
        ...
        cmd_cd = 'cd secure-distributed-linear-regression; pwd; git stash; git checkout master; make OBLIVCC=../obliv-c/bin/oblivcc; killall secure_multiplication'
    ...
    for d in [5, 25, 50, 250, 500]:
        for n in [100*d, 200*d, 2000*d]:
            for p in [2, 3, 4, 5]: # p is number of data providers (not TI)
                ...
                for i in range(args.num_examples):
                    ...
                    for party in range(p+1):
                        ...
                        cmd = '{0} {1} {2} > {3} 2>&1 {4}'.format(args.exec_file, filepath_lr_in, party, filepath_lr_out, '&' if party < p else '')
                        if args.ips_file:
