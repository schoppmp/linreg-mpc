...
static void random_vector_t(vector_t *vector){
	for(size_t i = 0; i < vector->len; i++) {
		gcry_randomize(&vector->value[i], sizeof(fixed32_t), GCRY_STRONG_RANDOM);
	}
}

static int receive_msg(vector_t *vector, fixed32_t *value, void *sender) {
	int *val = sender;
...
static int send_msg(vector_t *vector, fixed32_t value, void *recipient) {
	int status;
	int *rec = recipient;
	printf("Sending msg...\n");
...
int main(int argc, char **argv) {
	...
	if(party == 3) { // Trusted Initializer
		...
		vector_t x;
		x.len = d;
		x.value = malloc(d * sizeof(fixed32_t));
		random_vector_t(&x);
		vector_t y;
		y.len = d;
		y.value = malloc(d * sizeof(fixed32_t));
		random_vector_t(&y);
		fixed32_t r = 0;
		...
		input_fixed.len = d;
		input_fixed.value = malloc(d * sizeof(fixed32_t));
		...
		in_vector_1.len = d;
		in_vector_1.value = malloc(d * sizeof(fixed32_t));
		fixed32_t in_value_1;
		...
		if(party == 2){
		  	...
		  	vector_t out_vector;
		  	out_vector.len = d;
			out_vector.value = malloc(d * sizeof(fixed32_t));
			for(size_t i=0; i<input_fixed.len; ++i){
			...
			in_vector_2.len = d;
			in_vector_2.value = malloc(d * sizeof(fixed32_t));
			fixed32_t in_value_2;
			status = receive_msg(&in_vector_2, &in_value_2, socket[party-1]);
			check(status == 0, "Error receiving messages from party %d", 1);

			fixed32_t share = inner_product(&in_vector_2, &in_vector_1) + in_value_2 - in_value_1;
