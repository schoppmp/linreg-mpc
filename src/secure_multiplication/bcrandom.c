...
{ 
  ...
  for(i=0;i+blen<=len;i+=blen){
    gcry_cipher_encrypt(gen->cipher,(unsigned char*)dest+i,blen, gen->zeroes,blen);
  }
  if(i<len)
...
uint32_t bcRandom_uint32(BCipherRandomGen* gen, unsigned long long max)
{

  unsigned long long rv;
  randomizeBuffer(gen,(char*)&rv,sizeof(rv));
  return (uint32_t) rv%max; // so about that bias: max should be << 2^64
}
